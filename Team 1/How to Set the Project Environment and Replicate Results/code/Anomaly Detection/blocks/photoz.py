...
class Input_Mix_Model(nn.Module):
    def __init__(self, in_channel, out_channel, embed_dim, Inception_only, magnitude, uk_paper):
        ...
        self.conv2d_init = nn.Conv2d(in_channel,  out_channel, kernel_size = (1, 1), padding_mode = 'zeros')
    ...
    def vis_bulit(self):
        ...
        for i in range(5):
            if i != 4:
                layers.append(InceptionBlock(in_channels = self.config[i]['in_channel'], 
                                    ...
                                    is_custom = False).to(self.device))
