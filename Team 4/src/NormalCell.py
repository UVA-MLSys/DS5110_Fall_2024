...
class NormalCell(nn.Module):
    def __init__(
        ...
        mlp_hidden_dim = (
            mlp_hidden_dim if mlp_hidden_dim is not None else int(dim * mlp_ratio)
        )
    ...
    def forward(self, x):
        ...
        if self.class_token:
            ...
            convX = self.drop_path(
                self.PCM(
                    x[:, 1:, :].view(b, wh, wh, c).permute(0, 3, 1, 2).contiguous()
                )
                .permute(0, 2, 3, 1)
                .contiguous()
                .view(b, n, c)
            ...
            convX = self.drop_path(
                self.PCM(x_2d).permute(0, 2, 3, 1).contiguous().view(b, n, c)
            )
            x = x + self.drop_path(self.attn(self.norm1(x)))
            x = x + convX
